...
class PessoaViewTestCase(BaseAuthenticatedUserTestCase):
    def setUp(self):
        ...
        self.pessoa2SIE = PessoaSIE.objects.create(nome_pessoa="Shintaro Yamazaki", rg="22200000000", cpf="22200000000")
...
class SinglePessoaViewTestCase(BaseAuthenticatedUserTestCase):
    def setUp(self):
        ...
        self.id_pessoa_interna_sie = self.pessoa2SIE.id_pessoa_sie
        ...
        self.url = reverse(self.urlPrefix, kwargs={"pk": self.id_pessoa_interna_sie})

    def test_get_single_pessoa(self):
        self.authentication(self.user_data_adm)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["nome_pessoa"], "Leandro")
    ...
    def test_get_single_pessoa_only_registered_in_sie(self):
        self.authentication(self.user_data_adm)
        url = reverse(self.urlPrefix, kwargs={"pk": self.pessoa.id_pessoa_sie})
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
        self.assertEqual(response.data["nome_pessoa"], "Shintaro Yamazaki")
